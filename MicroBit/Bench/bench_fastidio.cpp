/**
 * bench_fastidio.cpp - BENCH DE HOST: Fastidio vieja vs. Fastidio con patron de frame.
 *
 ...
 * se flashea.
 *
 * Fastidio es el caso MECANICO: no tiene azar, todo sale de la tabla de
...
#include "Animaciones/Emociones/Fastidio/Fastidio.h"

#include <stdio.h>

void animarFastidioVieja();     // la anterior, de git
...
int main()
{
    const unsigned long CICLO_NUEVO = 1620;
    const unsigned long CICLO_VIEJO = 1620;

    printf("micro:bit bench - Fastidio (host, modelo Cortex-M0+ @64MHz)\n");
    printf("las dos versiones son codigo real (la vieja sale de git show)\n\n");

    Resultado v = medir(animarFastidioVieja, CICLO_VIEJO, false);
    Resultado n = medir(animarFastidio,      CICLO_NUEVO, true);
    ...
    for (unsigned long t = 30; t < CICLO_NUEVO; t += 29) {
        unsigned long x = medir_latencia(animarFastidioVieja, CICLO_VIEJO, false, t);
        unsigned long y = medir_latencia(animarFastidio,      CICLO_NUEVO, true,  t);
        if (x > peorV) peorV = x;
        if (y > peorN) peorN = y;
    }
    printf("\n  -- reactividad --\n");
