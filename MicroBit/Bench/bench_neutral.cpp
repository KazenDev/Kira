/**
 * bench_neutral.cpp - BENCH DE HOST: Neutral vieja vs. Neutral con patron de frame.
 *
 ...
 * se flashea.
 *
 * Neutral es el caso MECANICO: no tiene azar, todo sale de la tabla de
...
#include "Animaciones/Emociones/Neutral/Neutral.h"

#include <stdio.h>

void animarNeutralVieja();     // la anterior, de git
...
int main()
{
    const unsigned long CICLO_NUEVO = 3880;
    const unsigned long CICLO_VIEJO = 3880;

    printf("micro:bit bench - Neutral (host, modelo Cortex-M0+ @64MHz)\n");
    printf("las dos versiones son codigo real (la vieja sale de git show)\n\n");

    Resultado v = medir(animarNeutralVieja, CICLO_VIEJO, false);
    Resultado n = medir(animarNeutral,      CICLO_NUEVO, true);
    ...
    for (unsigned long t = 30; t < CICLO_NUEVO; t += 29) {
        unsigned long x = medir_latencia(animarNeutralVieja, CICLO_VIEJO, false, t);
        unsigned long y = medir_latencia(animarNeutral,      CICLO_NUEVO, true,  t);
        if (x > peorV) peorV = x;
        if (y > peorN) peorN = y;
    }
    printf("\n  -- reactividad --\n");
