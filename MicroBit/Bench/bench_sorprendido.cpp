/**
 * bench_sorprendido.cpp - BENCH DE HOST: Sorprendido vieja vs. Sorprendido con patron de frame.
 *
 ...
 * se flashea.
 *
 * Sorprendido es el caso MECANICO: no tiene azar, todo sale de la tabla de
 * segmentos y de la fase. Por eso es la de referencia para las
 * cinco que quedan sin azar.
 *
 * Uso:  ./bench_sorprendido
 */
#include "mock/MicroBit.h"
#include "Animaciones/Sistema/Sistema.h"
#include "Animaciones/Emociones/Sorprendido/Sorprendido.h"

#include <stdio.h>

void animarSorprendidoVieja();     // la anterior, de git
...
int main()
{
    const unsigned long CICLO_NUEVO = 2630;
    const unsigned long CICLO_VIEJO = 2630;

    printf("micro:bit bench - Sorprendido (host, modelo Cortex-M0+ @64MHz)\n");
    printf("las dos versiones son codigo real (la vieja sale de git show)\n\n");

    Resultado v = medir(animarSorprendidoVieja, CICLO_VIEJO, false);
    Resultado n = medir(animarSorprendido,      CICLO_NUEVO, true);
    ...
    for (unsigned long t = 30; t < CICLO_NUEVO; t += 29) {
        unsigned long x = medir_latencia(animarSorprendidoVieja, CICLO_VIEJO, false, t);
        unsigned long y = medir_latencia(animarSorprendido,      CICLO_NUEVO, true,  t);
        if (x > peorV) peorV = x;
        if (y > peorN) peorN = y;
    }
    printf("\n  -- reactividad --\n");
