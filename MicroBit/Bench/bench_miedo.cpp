/**
 * bench_miedo.cpp - BENCH DE HOST: Miedo vieja vs. Miedo con patron de frame.
 *
 ...
 * se flashea.
 *
 * Miedo es el caso MECANICO: no tiene azar, todo sale de la tabla de
...
#include "Animaciones/Emociones/Miedo/Miedo.h"

#include <stdio.h>

void animarMiedoVieja();     // la anterior, de git
...
int main()
{
    const unsigned long CICLO_NUEVO = 2210;
    const unsigned long CICLO_VIEJO = 2210;

    printf("micro:bit bench - Miedo (host, modelo Cortex-M0+ @64MHz)\n");
    printf("las dos versiones son codigo real (la vieja sale de git show)\n\n");

    Resultado v = medir(animarMiedoVieja, CICLO_VIEJO, false);
    Resultado n = medir(animarMiedo,      CICLO_NUEVO, true);
    ...
    for (unsigned long t = 30; t < CICLO_NUEVO; t += 29) {
        unsigned long x = medir_latencia(animarMiedoVieja, CICLO_VIEJO, false, t);
        unsigned long y = medir_latencia(animarMiedo,      CICLO_NUEVO, true,  t);
        if (x > peorV) peorV = x;
        if (y > peorN) peorN = y;
    }
    printf("\n  -- reactividad --\n");
