/**
 * bench_triste.cpp - BENCH DE HOST: Triste vieja vs. Triste con patron de frame.
 *
 ...
 * se flashea.
 *
 * Triste es el caso dificil de los ocho: es la unica con azar. El temblor del
 * labio venia de codal::random(), que es un LFSR con estado GLOBAL
 * compartido, y la lagrima se disparaba por un contador de pasadas. Con el
 * patron de frame el estado tiene que estar en el reloj.
 *
 * Ademas se mide una cosa que solo importa aca: que la lagrima siga cayendo
 * cada 4-7 ciclos y no cada frame.
 *
 * Uso:  ./bench_triste
 */
#include "mock/MicroBit.h"
#include "Animaciones/Sistema/Sistema.h"
#include "Animaciones/Emociones/Triste/Triste.h"

#include <stdio.h>

void animarTristeVieja();     // la anterior, de git
...
int main()
{
    const unsigned long CICLO_NUEVO = 6760;
    const unsigned long CICLO_VIEJO = 6000;   // sin lagrima

    printf("micro:bit bench - Triste (host, modelo Cortex-M0+ @64MHz)\n");
    printf("las dos versiones son codigo real (la vieja sale de git show)\n\n");

    Resultado v = medir(animarTristeVieja, CICLO_VIEJO, false);
    Resultado n = medir(animarTriste,      CICLO_NUEVO, true);
    ...
    for (unsigned long t = 30; t < CICLO_NUEVO; t += 29) {
        unsigned long x = medir_latencia(animarTristeVieja, CICLO_VIEJO, false, t);
        unsigned long y = medir_latencia(animarTriste,      CICLO_NUEVO, true,  t);
        if (x > peorV) peorV = x;
        if (y > peorN) peorN = y;
    }
    printf("\n  -- reactividad --\n");
    ...
    // La lagrima: cada cuanto cae. Es el reloj de CALENDARIO de la emocion y
    // es lo UNICO que el patron de frame no puede derivar del tiempo, asi que
    // hay que verificar a mano que sigue siendo "cada 4-7 ciclos".
    printf("\n  -- la lagrima (reloj de calendario) --\n");
    bench_reset();
    uBit.display.image.clear();
    mostrarCaraTriste();
    {
        const unsigned int N = 20;          // 20 ciclos
        unsigned long t0 = bench.nowMs;
        while (bench.nowMs - t0 < CICLO_NUEVO * N) animarTriste();
        unsigned int lagrimas = (unsigned int)bench.lagrimas;
        printf("  ciclos simulados            : %u\n", N);
        printf("  lagrimas que cayeron        : %u\n", lagrimas);
        if (lagrimas == 0) {
            printf("  FALLO: no cae ninguna lagrima\n");
        } else {
            printf("  una cada %.1f ciclos\n", (double)N / lagrimas);
            if ((double)N / lagrimas < 2.0)
                printf("  FALLO: cae demasiado seguido (contador bajado por frame?)\n");
            else if ((double)N / lagrimas > 12.0)
                printf("  AVISO: cae muy raramente\n");
            else
                printf("  OK: el calendario se mantiene (el original: cada 4-7)\n");
        }
    }
