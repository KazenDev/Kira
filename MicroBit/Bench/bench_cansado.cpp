/**
 * bench_cansado.cpp - BENCH DE HOST: Cansado vieja vs. Cansado con patron de frame.
 *
 ...
 * se flashea.
 *
 * Cansado es el caso MECANICO: no tiene azar, todo sale de la tabla de
 * segmentos y de la fase. Por eso es la migracion de referencia para las
...
#include "Animaciones/Emociones/Cansado/Cansado.h"

#include <stdio.h>

void animarCansadoVieja();     // la anterior, de git
...
int main()
{
    const unsigned long CICLO_NUEVO = 4330;
    const unsigned long CICLO_VIEJO = 4330;

    printf("micro:bit bench - Cansado (host, modelo Cortex-M0+ @64MHz)\n");
    printf("las dos versiones son codigo real (la vieja sale de git show)\n\n");

    Resultado v = medir(animarCansadoVieja, CICLO_VIEJO, false);
    Resultado n = medir(animarCansado,      CICLO_NUEVO, true);
    ...
    for (unsigned long t = 30; t < CICLO_NUEVO; t += 29) {
        unsigned long x = medir_latencia(animarCansadoVieja, CICLO_VIEJO, false, t);
        unsigned long y = medir_latencia(animarCansado,      CICLO_NUEVO, true,  t);
        if (x > peorV) peorV = x;
        if (y > peorN) peorN = y;
    }
    printf("\n  -- reactividad --\n");
